...
serve(async (req: Request) => {
  ...
  try {
    ...
    // ── 2. Fetch all active recurring tasks ────────────────────────────────
    ...
    // ── 3. Filter to those that should run today ───────────────────────────
    ...
    // ── 4. Find a "system" creator (any owner/admin in the same company) ───
    ...
    // ── 5. Check which recurring tasks already materialized today ──────────
    ...
    // ── 6. Insert new task rows for due recurring tasks ────────────────────
    ...
    console.error('[materialize-recurring-tasks] Materialization error:', error);
    return new Response(
